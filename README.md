...
This project is based on the video series of javidx9 (https://www.youtube.com/watch?v=2hNdkYInj4g&t=485s) and other ressources (within the ```other_ressources``` folder - https://theboostcpplibraries.com/boost.asio, https://medium.com/from-the-scratch/http-server-what-do-you-need-to-know-to-build-a-simple-http-server-from-scratch-d1ef8945e4fa) to familiarize myself with networking in C++ and on a very low level of abstraction and get a deeper understanding how to program networking applications. Also the most common issues are elaborated as well as discussed, how they could be solved, using the ```boost::asio``` library. In the last sections of the tutorial, a basic understanding about cyber security is given. The main takeaways are written down here in the ```README.md```.
The difference between my project and the one of javidx9 is, that he uses a Windows setup by incooperating Visual Studio and its build system, whereas I using a CMake project as my build system and working completly under linux (Ubuntu 20.04 LTS). Also, I want to use the networking framework/header based library to build my own chat application. So lets get started. ;-)  
