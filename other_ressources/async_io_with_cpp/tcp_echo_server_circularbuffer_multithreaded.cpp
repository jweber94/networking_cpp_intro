...
class IoContextGroup
{
    public: 
        IoContextGroup(std::size_t size){
            for (std::size_t n = 0; n < size; ++n){
                contexts_.emplace_back(); // emplace_back is the move semantics based .push_back() equivalent
                guards_.emplace_back(std::make_shared<work_guard_type>(contexts_.back()->get_executor()));  
            }
        }

        void run()
        {
            // let the I/O service objects run in the allocated threads in order to execute the completion handlers  
            for (auto & io_context : contexts_){
                threads_.emplace_back([&](){
                    io_context->run(); 
                }); 
            }

            // join the threads to avoid segmentation faults at the end of the program
        ...
        io::io_service & query() {
            // Get the next I/O service object for scheduling asynchronous tasks by incooperating the round robin load balancing algorithm 
    ...
    private:
        ...
        vector_ptr<io::io_service> contexts_;
        vector_ptr<work_guard_type> guards_; 
        std::vector<std::thread> threads_; 
        
        std::atomic<std::size_t> index_ = 0; // atomic means that everything that is done to the index_ instance will be thread safe or more precise, will be interpreted as an atomic operation by the OS
            // ref.: https://stackoverflow.com/questions/31978324/what-exactly-is-stdatomic
};
...
{
    public: 
        Server(IoContextGroup & ioservs, std::uint16_t port) : ioserv_group_(ioservs), acceptor_(ioserv, tcp::endpoint(tcp::v4(), port)){
            accept(); 
        }
    ...
    private: 
        // member methods
        void accept(){
            socket_.emplace(ioserv_group_.query()); // associate the next best I/O service object to the socket according to the round robin algorithm 
...
int main(int argc, char * argv[]){
    ...
    if (argc != 3){
        ...
        IoContextGroup ioservs(boost::lexical_cast<std::size_t>(argv[2])); 
        Server srv(ioservs, boost::lexical_cast<std::uint16_t>(argv[1]));
    }

    std::cout << "Finished server program on termination.\n"; 
    return 0; 
}
